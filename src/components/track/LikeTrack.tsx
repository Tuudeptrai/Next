...
const LikeTrack = (props: IProps) => {
    ...
    const handleLikeTrack = async () => {
       const res = await sendRequest<IBackendRes<IModelPaginate<ITrackLike>>>({
        ...
        fetchData();
        router.refresh();
        console.log('trackLike',trackLikes);
    }
    ...
    return (
        <div style={{ margin: "20px 10px 0 10px", display: "flex", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", width: "100px", gap: "20px", color: "#999" }}>
                ...
                <span style={{ display: "flex", alignItems: "center" }}><FavoriteIcon sx={{ fontSize: "20px" }} /> {
                //@ts-ignore trackLikes?.some(t => t.countLike !== track?.countLike)?trackLikes[0]?.countLike:
                 track?.countLike}</span>
            </div>
        </div>
    )
}
